...
{
    if (trim((string) MAIL_USERNAME) === '') {
        return false;
    }

    if (trim((string) MAIL_PASSWORD) === '') {
        return false;
    }

    if (trim((string) MAIL_FROM_EMAIL) === '') {
        return false;
    }
...
function send_otp_email($email, $name, $otpCode, $subject, $title, $message)
{
    if (!mail_settings_ready()) {
        throw new RuntimeException(
            'Set the sender email settings in .env first. User email is used automatically from the form.'
        );
    }
